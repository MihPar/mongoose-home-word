...
describe("/auth", () => {
  ...
  describe("/auth/password-recowery", async() => {
	...
	it("password is recovery", async() => {
		const recoveryPassword = await request(app).post("/auth/password-recovery").send({"emaiil": "mpara7274@gmail.com"})
		expect(recoveryPassword.status).toBe(HTTP_STATUS.NO_CONTENT_204)
	})

	it("input date is invalide", async() => {
		const recoveryPassword = await request(app).post("/auth/password-recovery").send({"emaiil": "mpara7274#mail.com"})
		expect(recoveryPassword.status).toBe(HTTP_STATUS.BAD_REQUEST_400)
	})

	let recoveryPassword 
	it("more then five attempts", async() => {
		recoveryPassword = await request(app).post("/auth/password-recovery").send({"emaiil": "mpara274@gmail.com"})
		recoveryPassword = await request(app).post("/auth/password-recovery").send({"emaiil": "mpara74@gmail.com"})
		recoveryPassword = await request(app).post("/auth/password-recovery").send({"emaiil": "mpara4@gmail.com"})
		recoveryPassword = await request(app).post("/auth/password-recovery").send({"emaiil": "mpara7@gmail.com"})
		recoveryPassword = await request(app).post("/auth/password-recovery").send({"emaiil": "mpara@gmail.com"})
		recoveryPassword = await request(app).post("/auth/password-recovery").send({"emaiil": "mpar74@gmail.com"})
  ...
  describe("/auth/new-password", async() => {
	it("confirm password recovery", async() => {
		const recoveryPassword = await request(app).post('/auth/recovery-password').send({
	...
	it("input data is incorrect", async() => {
		await request(app).post('/auth/recovery-password').send({
